...
The main object of the `headr` package is to process titles (headers) and footnotes file and to offer an interface with commonly used table packages (e.g., `flextable`) or figure packages (e.g., `ggplot2`) to add titles and footers to clinical study reports (CSR) deliverables.
